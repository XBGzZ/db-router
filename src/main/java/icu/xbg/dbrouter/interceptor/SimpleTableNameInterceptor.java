...
public class SimpleTableNameInterceptor extends ConnectProxy {
    ...
    public SimpleTableNameInterceptor(Connection connection, DBProperties properties, MetaResolver resolver, StrategyCache cache) {
    ...
    protected String modifyTableName(String sql, Integer tbCount, String dbName, String[] tbKeys, String suffix) {
        String result = sql;
        if (tbCount > 1 && StringUtils.hasLength(suffix)) {
            if (Objects.nonNull(tbKeys)) {
